...
# List resources
RSC=`find resources -type f -print | awk '{print "./" $0}'`
echo $RSC | sed -e 's/ /\n/g' | wixl-heat --directory-ref INSTALLDIR --component-group BaseResources -p ./ --var sys.CURRENTDIR | sed -e '1,2d' | head -n -3 >> "${2}/p_post.wxs"
